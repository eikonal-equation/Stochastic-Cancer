function Xcost = CDF_threshold_aware_policy_withMTD(Nx,D_thres_filepath,Initial_budget,...
...
% Here we use the MTD-based therapy after the budget runs out.)
%
%Nx (input): Number of points along one side of the spatial grid
%
...
D_thres = memmapfile(D_thres_filepath,'Format',{'uint8',[Nx+1,Nx+1,Total_Num_DataSlices],'dd'})
...
parfor ii = 1:sample_size
    ...
    while ylist(j) >= r_b
        ...
        if ((next_budget < 0) || abs(next_budget -0) < 10^(-5))
            % using MTD-based therapy after running out of budget
            policy = dmax;
            dt = 0.001;

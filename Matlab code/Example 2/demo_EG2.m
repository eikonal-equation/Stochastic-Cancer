%% Optimal cancer therapy under stochasticity Demo file (EG2: SR model)
%
...
% we've used a 1601x1601 uniform grid on the unit qp-square in our actual
...
tic
[U,Dmat_det] = Deterministic_Cancer_ValuePolicy_Ite_EG2(N_det);
t1 = toc;
...
% policy date file.
filepath = 'test_policy_eg2_400.dat';
Total_num_s_slices = 601;
...
tic
Xcost_stationary = CDF_stationary_policy_EG2(Dmat_det,xloc,yloc,Initial_policy,choice,sample_size);
t2 = toc;
...
%
% Set up the initial budget sbar (time-rescaled)
sbar = 4;
Sbar = 6;
% Set up the initial policy
Initial_policy = 0;
...
Xcost_thres = CDF_threshold_aware_policy_EG2(Dmat_det,filepath,sbar,...
    Initial_policy,xloc,yloc,choice,choice_bd,sample_size,Total_num_s_slices,Sbar);
